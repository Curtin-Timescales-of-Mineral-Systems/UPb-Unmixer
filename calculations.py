...
from config import SIGMAS_OUTPUT_ERROR
...
def age_from_u238pb206(u238pb206):
    return error.log(1/u238pb206 + 1)/U238_DECAY_CONSTANT

def pb206u238_from_age(age):
    return error.exp(U238_DECAY_CONSTANT * age) - 1

def u238pb206_from_age(age):
    return 1/(pb206u238_from_age(age))

def pb207u235_from_age(age):
    return error.exp(U235_DECAY_CONSTANT * age) - 1
...
def reconstruct_age(x1, y1, x2, y2):
    m = (y2 - y1)/(x2 - x1)
    c = y1 - m*x1

    lower_limit = age_from_u238pb206(min(error.value(x1),error.value(x2)))
    upper_limit = 5*(10**9)
    ...
    def solve_for_t(error_sign):
        def func(t):
            ...
            line_pb207pb206_value = error.value(line_pb207pb206) + error_sign*SIGMAS_OUTPUT_ERROR*error.stddev(line_pb207pb206)
